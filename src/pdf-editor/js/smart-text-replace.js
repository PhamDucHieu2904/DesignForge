/* ══════════════════════════════════════════════
   smart-text-replace.js — Advanced Text Replacement
   Nhận diện và render hoàn toàn trong trình duyệt; không upload PDF.
   ══════════════════════════════════════════════ */

// ── State ──
let _smartTextActive = false;         // Tool SmartText đang bật?
let _smartTextDragState = null;       // Trạng thái kéo vùng chọn
let _smartTextSelectionEl = null;     // DOM element vùng chọn
let _smartTextOverlayDimEl = null;    // DOM element làm tối xung quanh
let _smartTextSelectionRect = null;   // { x, y, w, h } trong tọa độ overlay
let _smartTextEditingObjectId = null; // Smart Text object đang được chỉnh lại

/* ════════════════════════════════════════════
   KHỞI TẠO — gọi từ initEditPDF()
   ════════════════════════════════════════════ */
function initSmartTextTool() {
  const btn = document.getElementById('smart-text-fill-btn');
  if (!btn) return;
  ...
  btn.addEventListener('click', async () => {
    ...
    _smartTextEditingObjectId = null;
    _smartTextActive = true;
...
function _openSmartTextObjectEditor(obj, pg, requestedMode) {
  ...
  _smartTextEditingObjectId = obj.id;
  _smartTextSelectionRect = {
    x: sourceRectPt.x * editorScale,
    y: sourceRectPt.y * editorScale,
    w: sourceRectPt.w * editorScale,
    h: sourceRectPt.h * editorScale
  };
  _showSmartTextPromptDialog(obj, requestedMode);
}

/* ════════════════════════════════════════════
   BIND SỰ KIỆN VẼ VÙNG CHỌN TRÊN CANVAS
   ════════════════════════════════════════════ */
function _bindSmartTextCanvasEvents() {
  const canvasArea = document.getElementById('edit-canvas-area');
  if (!canvasArea) return;

  function onMousedown(e) {
    if (!_smartTextActive) return;
    ...
    _removeSmartTextSelectionEl();

    const selEl = document.createElement('div');
    selEl.id = 'smart-text-selection-box';
    ...
    function onMove(e2) {
      ...
      _smartTextSelectionRect = { x, y, w, h };
    }
    ...
    function onUp(e2) {
      ...
      if (!_smartTextSelectionRect || _smartTextSelectionRect.w < minLogicalSize || _smartTextSelectionRect.h < minLogicalSize) {
        _removeSmartTextSelectionEl();
        return;
      }

      // Hiển thị dialog nhập prompt
      _showSmartTextPromptDialog();
    }

    document.addEventListener('mousemove', onMove);
    document.addEventListener('mouseup', onUp);

    _smartTextDragState = { onMove, onUp };
  }

  canvasArea.addEventListener('mousedown', onMousedown);
  // Lưu ref để có thể remove sau
  canvasArea._smartTextMousedown = onMousedown;
}

/* ════════════════════════════════════════════
   DIALOG NHẬP PROMPT
   ════════════════════════════════════════════ */
function _showSmartTextPromptDialog(editingObj = null, requestedRenderMode = null) {
  // Remove dialog cũ nếu có
  document.getElementById('smart-text-prompt-dialog')?.remove();
  const editingSmartText = editingObj?.smartText || null;
  if (!editingSmartText) _smartTextEditingObjectId = null;
  const initialRenderMode = requestedRenderMode === 'vector' || requestedRenderMode === 'raster'
    ? requestedRenderMode
    : (editingSmartText?.renderMode || 'raster');

  const dialog = document.createElement('div');
  dialog.id = 'smart-text-prompt-dialog';
  dialog.innerHTML = `
    <div class="smart-text-dialog-backdrop"></div>
    <div class="smart-text-dialog-box">
      <div class="smart-text-dialog-header">
        <span class="smart-text-dialog-icon">
          <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
            <path d="M12 20h9"></path>
            <path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path>
          </svg>
        </span>
        <span class="smart-text-dialog-title">${editingSmartText ? 'Chỉnh Lại Chữ Nâng Cao' : 'Sửa Chữ Nâng Cao'}</span>
      </div>
      <div class="smart-text-dialog-body" style="padding-top: 10px;">
        
        <!-- Mode: Text -->
        <div id="smart-text-mode-text" style="display: block;">
          <div class="smart-text-render-mode-row">
            <label for="smart-text-render-mode">Kiểu kết quả</label>
            <select id="smart-text-render-mode">
            ...
            <span id="smart-text-render-mode-note"></span>
          </div>
          <div class="smart-text-font-detect-panel">
            <div class="smart-text-detect-status-row">
              <span id="smart-text-font-detect-status" class="smart-text-detect-status">Đang chuẩn bị nhận diện font…</span>
              <span class="smart-text-detect-progress"><i id="smart-text-font-detect-progress"></i></span>
            </div>
            <div class="smart-text-source-row">
              <label for="smart-text-source-text">Chữ gốc</label>
              <input id="smart-text-source-text" type="text" placeholder="Tự đọc từ PDF hoặc OCR" autocomplete="off">
              <button id="smart-text-font-redetect" class="smart-text-redetect-btn" type="button">Dò lại</button>
            </div>
            <div id="smart-text-font-candidates" class="smart-text-font-candidates"></div>
          </div>
          <p class="smart-text-dialog-hint">Nhập chữ mới bạn muốn chèn vào bản scan</p>
          <textarea id="smart-text-prompt-input" class="smart-text-prompt-textarea" 
            placeholder="Ví dụ: THIẾT KẾ VINUT"
            rows="2" spellcheck="false"></textarea>
          
          <div class="smart-text-font-controls" style="margin-top: 8px; display: flex; gap: 8px; font-size: 12px; align-items: center;">
            <select id="smart-text-font-family" style="padding: 4px; border: 1px solid #ccc; border-radius: 4px; outline: none; background: #fff; flex: 1;">
              <option value="Arial">Arial</option>
            </select>
            <select id="smart-text-font-weight" style="padding: 4px; border: 1px solid #ccc; border-radius: 4px; outline: none; background: #fff; width: 80px;">
              <option value="normal">Thường</option>
              <option value="bold" selected>Đậm</option>
            </select>
            <select id="smart-text-font-style" style="padding: 4px; border: 1px solid #ccc; border-radius: 4px; outline: none; background: #fff; width: 85px;">
              <option value="normal">Thẳng</option>
              <option value="italic">Nghiêng</option>
            </select>

            <input type="color" id="smart-text-text-color" value="#000000" style="padding: 0; border: 1px solid #ccc; border-radius: 4px; height: 26px; width: 30px; cursor: pointer;" title="Màu chữ">
          </div>
          
          <div class="smart-text-text-adjustments" style="margin-top: 12px; font-size: 12px; color: #555;">
            <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
              <span>Cỡ Chữ (Size): <span id="smart-text-font-size-val">70</span>%</span>
              <input type="range" id="smart-text-font-size-slider" min="10" max="150" step="1" value="70" style="width: 120px;" title="Tỷ lệ cỡ chữ so với khung chọn">
            </div>
          </div>

          <div class="smart-text-appearance-panel">
            <div class="smart-text-appearance-heading">
              <label for="smart-text-appearance-mode">Hiệu ứng chữ</label>
              <select id="smart-text-appearance-mode">
            ...
            <div id="smart-text-scan-profile-status" class="smart-text-scan-profile-status">Đang phân tích hiệu ứng scan…</div>
            <div class="smart-text-effect-grid">
              <label><span>Cường độ <b id="smart-text-scan-strength-val">80</b>%</span><input id="smart-text-scan-strength" type="range" min="0" max="150" step="1" value="80"></label>
              <label><span>Blur <b id="smart-text-blur-val">0.0</b>px</span><input id="smart-text-blur-slider" type="range" min="0" max="3" step="0.1" value="0"></label>
              <label><span>Sharpen <b id="smart-text-sharpen-val">0</b>%</span><input id="smart-text-sharpen-slider" type="range" min="0" max="150" step="1" value="0"></label>
              <label><span>Mực loang <b id="smart-text-spread-val">0</b>%</span><input id="smart-text-spread-slider" type="range" min="-100" max="100" step="1" value="0"></label>
              <label><span>Contrast <b id="smart-text-contrast-val">100</b>%</span><input id="smart-text-contrast-slider" type="range" min="70" max="140" step="1" value="100"></label>
              <label><span>Noise <b id="smart-text-noise-val">0</b>%</span><input id="smart-text-noise-slider" type="range" min="0" max="100" step="1" value="0"></label>
              <label><span>JPEG <b id="smart-text-jpeg-val">100</b>%</span><input id="smart-text-jpeg-slider" type="range" min="60" max="100" step="1" value="100"></label>
              <label title="Làm nhạt lõi nét và gom mật độ mực về hai mép trong như Photoshop Smart Sharpen"><span>Smart sharpen <b id="smart-text-smart-sharpen-val">0</b>%</span><input id="smart-text-smart-sharpen-slider" type="range" min="0" max="100" step="1" value="0"></label>
            </div>
            <input id="smart-text-bg-noise" type="hidden" value="0">
            <input id="smart-text-ink-noise" type="hidden" value="0">
          </div>
          
          <div id="smart-text-live-preview-container" style="margin-top: 12px; text-align: center; border: 1px dashed #ccc; padding: 4px; border-radius: 4px; min-height: 40px; display: flex; align-items: center; justify-content: center; background: #fafafa; overflow: hidden;">
             <span style="color: #999; font-size: 11px;">Bản xem trước sẽ hiện ở đây...</span>
          </div>
        </div>

        <div class="smart-text-dialog-info" style="margin-top: 12px;">
          <span class="smart-text-info-icon">📐</span>
          <span id="smart-text-selection-size"></span>
        </div>
      </div>
      <div class="smart-text-dialog-footer">
        <button id="smart-text-cancel-btn" class="smart-text-btn smart-text-btn-cancel">Cancel</button>
        <button id="smart-text-create-btn" class="smart-text-btn smart-text-btn-create">
          <span class="smart-text-btn-icon">
  ...
  if (typeof populateSmartFontSelect === 'function') {
    populateSmartFontSelect(document.getElementById('smart-text-font-family'));
  }
  if (editingSmartText) _applySmartTextSettingsToDialog(editingSmartText, initialRenderMode);
  _syncSmartTextRenderModeUI();

  // Hiển thị kích thước vùng chọn
  const pg = _getCurrentPg();
  if (_smartTextSelectionRect && pg) {
    const sizeEl = document.getElementById('smart-text-selection-size');
    if (sizeEl) {
      ...
      sizeEl.textContent = `Vùng chọn: ${Math.round(_smartTextSelectionRect.w)} × ${Math.round(_smartTextSelectionRect.h)} px canvas · nguồn ${ppiLabel} PPI (${detectLabel})`;
    }
  }
  ...
  if (pg && _smartTextSelectionRect) {
    _captureSelectionRegion(pg, _smartTextSelectionRect).then(async base64 => {
      capturedBase64ForPreview = base64;

      if (editingSmartText) {
        detectedScanProfile = editingSmartText;
        const statusEl = document.getElementById('smart-text-font-detect-status');
        const progressEl = document.getElementById('smart-text-font-detect-progress');
      ...
      const detectionPromise = typeof analyzeSelectionFont === 'function'
        ? analyzeSelectionFont(pg, _smartTextSelectionRect, base64, updateDetectProgress)
      ...
      const colorInput = document.getElementById('smart-text-text-color');
      if (colorInput) colorInput.value = detectedColor;

      // Auto-detect Text Height
      const fontSizeSlider = document.getElementById('smart-text-font-size-slider');
      const fontSizeVal = document.getElementById('smart-text-font-size-val');
      ...
      if (scanProfile && document.getElementById('smart-text-appearance-mode')?.value === 'match') {
        _applyScanProfileToUI(scanProfile);
      } else if (!scanProfile) {
        const profileStatus = document.getElementById('smart-text-scan-profile-status');
        if (profileStatus) profileStatus.textContent = 'Không đủ dữ liệu để khớp tự động; có thể chỉnh tay.';
      }

      if (fontAnalysis) {
        const sourceInput = document.getElementById('smart-text-source-text');
        if (sourceInput) sourceInput.value = fontAnalysis.text || '';
        _renderSmartTextFontCandidates(fontAnalysis.candidates || [], fontAnalysis.source);
        if (fontAnalysis.candidates?.[0]) await _applySmartTextFontCandidate(fontAnalysis.candidates[0]);
  ...
  function _applyScanProfileToUI(profile) {
    if (!profile) return;
    _setEffectControl('smart-text-blur-slider', profile.blurPx, 'smart-text-blur-val', Number(profile.blurPx).toFixed(1));
    _setEffectControl('smart-text-sharpen-slider', Math.round(profile.sharpenAmount * 100), 'smart-text-sharpen-val');
    const safeAutoSpread = Math.max(-0.08, Math.min(0.08, Number(profile.inkSpread) || 0));
    _setEffectControl('smart-text-spread-slider', Math.round(safeAutoSpread * 100), 'smart-text-spread-val');
    _setEffectControl('smart-text-contrast-slider', Math.round(profile.contrast * 100), 'smart-text-contrast-val');
    _setEffectControl('smart-text-noise-slider', Math.round(profile.noiseAlpha * 100), 'smart-text-noise-val');
    _setEffectControl('smart-text-jpeg-slider', Math.round(profile.jpegQuality * 100), 'smart-text-jpeg-val');
    _setEffectControl('smart-text-smart-sharpen-slider', Math.round((Number(profile.smartSharpen) || 0) * 100), 'smart-text-smart-sharpen-val');
    const bgNoise = document.getElementById('smart-text-bg-noise');
    const inkNoise = document.getElementById('smart-text-ink-noise');
    if (bgNoise) bgNoise.value = profile.backgroundNoise || 0;
    if (inkNoise) inkNoise.value = profile.inkNoise || 0;
    const status = document.getElementById('smart-text-scan-profile-status');
  ...
  function _applyCleanProfileToUI() {
    _setEffectControl('smart-text-blur-slider', 0, 'smart-text-blur-val', '0.0');
    _setEffectControl('smart-text-sharpen-slider', 0, 'smart-text-sharpen-val');
    _setEffectControl('smart-text-spread-slider', 0, 'smart-text-spread-val');
    _setEffectControl('smart-text-contrast-slider', 100, 'smart-text-contrast-val');
    _setEffectControl('smart-text-noise-slider', 0, 'smart-text-noise-val');
    _setEffectControl('smart-text-jpeg-slider', 100, 'smart-text-jpeg-val');
    _setEffectControl('smart-text-smart-sharpen-slider', 0, 'smart-text-smart-sharpen-val');
    const bgNoise = document.getElementById('smart-text-bg-noise');
    const inkNoise = document.getElementById('smart-text-ink-noise');
  ...
  async function _applySmartTextFontCandidate(candidate) {
    if (!candidate) return;
    const family = document.getElementById('smart-text-font-family');
    const weight = document.getElementById('smart-text-font-weight');
    const style = document.getElementById('smart-text-font-style');
    ...
    if (typeof ensureGoogleFontLoaded === 'function') {
      await ensureGoogleFontLoaded(candidate.family, document.getElementById('smart-text-source-text')?.value || 'Tiếng Việt');
    }
    document.querySelectorAll('.smart-text-font-candidate').forEach(btn => btn.classList.toggle('active', btn.dataset.family === candidate.family));
    _updateLivePreview();
  }

  function _renderSmartTextFontCandidates(candidates, sourceKind) {
    const container = document.getElementById('smart-text-font-candidates');
    if (!container) return;
    container.innerHTML = '';
    candidates.slice(0, 3).forEach(candidate => {
      const button = document.createElement('button');
      button.type = 'button';
      button.className = 'smart-text-font-candidate';
      ...
      button.addEventListener('click', () => _applySmartTextFontCandidate(candidate));
  ...
  async function _updateLivePreview() {
    ...
    const container = document.getElementById('smart-text-live-preview-container');
    if (!container) return;

    const prompt = document.getElementById('smart-text-prompt-input')?.value?.trim();
    if (!prompt) {
    ...
    const manualStyle = { 
      fontFamily: document.getElementById('smart-text-font-family')?.value || 'Arial', 
      fontWeight: document.getElementById('smart-text-font-weight')?.value || 'bold', 
      fontStyle: document.getElementById('smart-text-font-style')?.value || 'normal',
      textAlign: 'left',
      textColor: document.getElementById('smart-text-text-color')?.value || '#000000',
      fontSizePct: parseFloat(document.getElementById('smart-text-font-size-slider')?.value || '70') / 100,
      sourceText: document.getElementById('smart-text-source-text')?.value?.trim() || '',
      renderMode: document.getElementById('smart-text-render-mode')?.value || 'raster',
      ..._readSmartTextScanEffectControls()
    };
    ...
    if (sequence !== livePreviewSequence || !document.getElementById('smart-text-prompt-dialog')) return;
    if (manualStyle.renderMode === 'vector') {
      _renderSmartTextVectorPreview(container, renderObj);
    } else {
  ...
  setTimeout(() => {
    const input = document.getElementById('smart-text-prompt-input');
    if (input) {
      input.focus();
  ...
  if (fontSizeSlider) fontSizeSlider.addEventListener('input', e => { 
    document.getElementById('smart-text-font-size-val').textContent = e.target.value; 
    _updateLivePreview();
  });
  
  const effectSliders = [
    ['smart-text-scan-strength','smart-text-scan-strength-val',false],
    ['smart-text-blur-slider','smart-text-blur-val',true],
    ['smart-text-sharpen-slider','smart-text-sharpen-val',true],
    ['smart-text-spread-slider','smart-text-spread-val',true],
    ['smart-text-contrast-slider','smart-text-contrast-val',true],
    ['smart-text-noise-slider','smart-text-noise-val',true],
    ['smart-text-jpeg-slider','smart-text-jpeg-val',true],
    ['smart-text-smart-sharpen-slider','smart-text-smart-sharpen-val',true]
  ];
  effectSliders.forEach(([inputId, valueId, markManual]) => {
    document.getElementById(inputId)?.addEventListener('input', e => {
      const display = document.getElementById(valueId);
      if (display) display.textContent = inputId === 'smart-text-blur-slider' ? Number(e.target.value).toFixed(1) : e.target.value;
      if (markManual) {
        const mode = document.getElementById('smart-text-appearance-mode');
  ...
  document.getElementById('smart-text-appearance-mode')?.addEventListener('change', e => {
  ...
  const fontControls = dialog.querySelectorAll('#smart-text-font-family, #smart-text-font-weight, #smart-text-font-style, #smart-text-text-color');
  ...
  document.getElementById('smart-text-render-mode')?.addEventListener('change', () => {
    _syncSmartTextRenderModeUI();
    _updateLivePreview();
  });

  document.getElementById('smart-text-font-redetect')?.addEventListener('click', async () => {
    const sourceText = document.getElementById('smart-text-source-text')?.value?.trim();
    if (!capturedBase64ForPreview || !sourceText || typeof detectNearestGoogleFonts !== 'function') return;
    const button = document.getElementById('smart-text-font-redetect');
    ...
    try {
      ...
      _renderSmartTextFontCandidates(candidates, 'manual');
      if (candidates[0]) await _applySmartTextFontCandidate(candidates[0]);
    } catch (err) {
      const statusEl = document.getElementById('smart-text-font-detect-status');
  ...
  function onKeydown(e) {
    if (e.key === 'Escape') { _handleSmartTextCancel(); document.removeEventListener('keydown', onKeydown); }
    if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) { _handleSmartTextCreate(); document.removeEventListener('keydown', onKeydown); }
  }
  document.addEventListener('keydown', onKeydown);
  dialog._keydownRef = onKeydown;

  // Bind buttons
  document.getElementById('smart-text-cancel-btn')?.addEventListener('click', () => {
    document.removeEventListener('keydown', onKeydown);
    _handleSmartTextCancel();
  });
  document.getElementById('smart-text-create-btn')?.addEventListener('click', () => {
    document.removeEventListener('keydown', onKeydown);
    _handleSmartTextCreate();
  });
}
...
function _applySmartTextSettingsToDialog(smartText, renderMode) {
  ...
  setValue('smart-text-source-text', smartText.sourceText || '');
  setValue('smart-text-prompt-input', smartText.replacementText || '');
  setValue('smart-text-font-family', smartText.fontFamily || 'Arial');
  setValue('smart-text-font-weight', smartText.fontWeight || 'normal');
  setValue('smart-text-font-style', smartText.fontStyle || 'normal');
  setValue('smart-text-text-color', smartText.textColor || '#000000');
  setValue('smart-text-render-mode', renderMode || smartText.renderMode || 'raster');

  const sizePct = Math.round((Number(smartText.fontSizePct) || 0.7) * 100);
  setValue('smart-text-font-size-slider', sizePct); setText('smart-text-font-size-val', sizePct);
  ...
  setValue('smart-text-appearance-mode', smartText.appearanceMode || 'match');
  setValue('smart-text-scan-strength', strength); setText('smart-text-scan-strength-val', strength);
  setValue('smart-text-blur-slider', blur); setText('smart-text-blur-val', blur.toFixed(1));
  setValue('smart-text-sharpen-slider', sharpen); setText('smart-text-sharpen-val', sharpen);
  setValue('smart-text-spread-slider', spread); setText('smart-text-spread-val', spread);
  setValue('smart-text-contrast-slider', contrast); setText('smart-text-contrast-val', contrast);
  setValue('smart-text-noise-slider', noise); setText('smart-text-noise-val', noise);
  setValue('smart-text-jpeg-slider', jpeg); setText('smart-text-jpeg-val', jpeg);
  setValue('smart-text-smart-sharpen-slider', smartSharpen); setText('smart-text-smart-sharpen-val', smartSharpen);
  setValue('smart-text-bg-noise', Number(smartText.backgroundNoise) || 0);
  setValue('smart-text-ink-noise', Number(smartText.inkNoise) || 0);
  const profileStatus = document.getElementById('smart-text-scan-profile-status');
  if (profileStatus) profileStatus.textContent = 'Đã nạp lại toàn bộ thông số của object hiện tại.';
}

function _syncSmartTextRenderModeUI() {
  const dialog = document.getElementById('smart-text-prompt-dialog');
  const mode = document.getElementById('smart-text-render-mode')?.value || 'raster';
  const note = document.getElementById('smart-text-render-mode-note');
  dialog?.classList.toggle('smart-text-vector-mode', mode === 'vector');
  if (note) {
...
function _renderSmartTextVectorPreview(container, renderObj) {
  ...
  wrapper.className = 'smart-text-vector-preview';
...
function _readSmartTextScanEffectControls() {
  return {
    appearanceMode: document.getElementById('smart-text-appearance-mode')?.value || 'match',
    scanStrength: parseFloat(document.getElementById('smart-text-scan-strength')?.value || '80') / 100,
    blurPx: parseFloat(document.getElementById('smart-text-blur-slider')?.value || '0'),
    sharpenAmount: parseFloat(document.getElementById('smart-text-sharpen-slider')?.value || '0') / 100,
    inkSpread: parseFloat(document.getElementById('smart-text-spread-slider')?.value || '0') / 100,
    contrast: parseFloat(document.getElementById('smart-text-contrast-slider')?.value || '100') / 100,
    noiseAlpha: parseFloat(document.getElementById('smart-text-noise-slider')?.value || '0') / 100,
    backgroundNoise: parseFloat(document.getElementById('smart-text-bg-noise')?.value || '0'),
    inkNoise: parseFloat(document.getElementById('smart-text-ink-noise')?.value || '0'),
    jpegQuality: parseFloat(document.getElementById('smart-text-jpeg-slider')?.value || '100') / 100,
    smartSharpen: parseFloat(document.getElementById('smart-text-smart-sharpen-slider')?.value || '0') / 100
  };
}
...
/* ════════════════════════════════════════════
   XỬ LÝ CANCEL
   ════════════════════════════════════════════ */
function _handleSmartTextCancel() {
  document.getElementById('smart-text-prompt-dialog')?.remove();
  _removeSmartTextSelectionEl();
  _cancelSmartTextTool();
  _smartTextEditingObjectId = null;
...
/* ════════════════════════════════════════════
   ...
   ════════════════════════════════════════════ */
async function _handleSmartTextCreate() {
  const pg = _getCurrentPg();
  if (!pg || !_smartTextSelectionRect) return;
  const editingObj = _smartTextEditingObjectId
    ? pg.overlayObjects.find(item => item.id === _smartTextEditingObjectId && item.smartText)
    : null;
  ...
  const prompt = document.getElementById('smart-text-prompt-input')?.value?.trim();
  if (!prompt) {
    document.getElementById('smart-text-prompt-input')?.focus();
    return;
  }

  // Đổi sang trạng thái loading
  _setSmartTextDialogLoading(true);

  try {
    // ── 1. Capture vùng chọn từ background image của page ──
    const capture = await _captureSelectionRegionDetailed(pg, _smartTextSelectionRect);
    const capturedBase64 = capture.base64;

    const manualFontFamily = document.getElementById('smart-text-font-family')?.value || 'Arial';
    const manualFontWeight = document.getElementById('smart-text-font-weight')?.value || 'bold';
    const manualFontStyle = document.getElementById('smart-text-font-style')?.value || 'normal';
    const manualTextAlign = 'left';
    const manualTextColor = document.getElementById('smart-text-text-color')?.value || '#000000';
    const manualFontSizePct = parseFloat(document.getElementById('smart-text-font-size-slider')?.value || '70') / 100;
    const renderMode = document.getElementById('smart-text-render-mode')?.value === 'vector' ? 'vector' : 'raster';
    const scanEffects = _readSmartTextScanEffectControls();
    const detectedSourceText = document.getElementById('smart-text-source-text')?.value?.trim() || '';
    ...
    const smartText = {
      ...
      sourceRect: { ..._smartTextSelectionRect },
    ...
    console.error('[SmartText Fill] Error:', err);
    _setSmartTextDialogLoading(false);
    _setSmartTextDialogError(err.message || 'Không thể xử lý vùng chữ. Vui lòng thử lại.');
  }
}
...
async function _captureSelectionRegionFromPreview(pg, rect) {
  return new Promise((resolve, reject) => {
    ...
    img.onerror = () => {
      // Nếu không load được img (CORS), tạo canvas trắng vẫn gọi SmartText được
      const canvas = document.createElement('canvas');
...
/* ════════════════════════════════════════════
   UI HELPERS
   ════════════════════════════════════════════ */
function _setSmartTextDialogLoading(loading) {
  const createBtn = document.getElementById('smart-text-create-btn');
  const cancelBtn = document.getElementById('smart-text-cancel-btn');
  const textarea  = document.getElementById('smart-text-prompt-input');
  const dialog    = document.getElementById('smart-text-prompt-dialog');
  ...
  if (loading) {
    ...
    createBtn.innerHTML = `<span class="smart-text-spinner"></span> ${_smartTextEditingObjectId ? 'Đang cập nhật...' : 'Đang tạo...'}`;
    if (cancelBtn) cancelBtn.disabled = true;
    if (textarea)  textarea.disabled = true;
    if (dialog) dialog.classList.add('loading');
  } else {
    createBtn.disabled = false;
    createBtn.innerHTML = `<span class="smart-text-btn-icon"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"></path><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg></span> ${_smartTextEditingObjectId ? 'Cập nhật' : 'Thay Chữ'}`;
...
function _setSmartTextDialogError(message) {
  const existing = document.querySelector('.smart-text-dialog-error');
  if (existing) existing.remove();

  const errEl = document.createElement('div');
  errEl.className = 'smart-text-dialog-error';
  errEl.textContent = '⚠ ' + message;

  const footer = document.querySelector('.smart-text-dialog-footer');
  ...
  if (createBtn) {
    createBtn.disabled = false;
    createBtn.innerHTML = `<span class="smart-text-btn-icon"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"></path><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg></span> Thử lại`;
  }
  const cancelBtn = document.getElementById('smart-text-cancel-btn');
  if (cancelBtn) cancelBtn.disabled = false;
  const textarea = document.getElementById('smart-text-prompt-input');
...
function _removeSmartTextSelectionEl() {
  if (_smartTextSelectionEl) {
    _smartTextSelectionEl.remove();
    _smartTextSelectionEl = null;
  }
  _smartTextSelectionRect = null;
}

function _cancelSmartTextTool() {
  _smartTextActive = false;
  _smartTextDragState = null;
  ...
  if (canvasArea) {
    ...
    if (canvasArea._smartTextMousedown) {
      canvasArea.removeEventListener('mousedown', canvasArea._smartTextMousedown);
      canvasArea._smartTextMousedown = null;
    }
  }
