...
async function _createBlurredEdgeImage(dataURL) {
  ...
  const setDownloadBusy = (busy, label = 'Đang xử lý…') => {
    if (dlBtn) {
      dlBtn.disabled = busy;
      dlBtn.textContent = busy ? label : 'Xuất PDF';
    }
...
function _updateCanvasPanAvailability() {
  const area = document.getElementById('edit-canvas-area');
  if (!area) return;
  const smartTextIsActive = typeof _smartTextActive !== 'undefined' && _smartTextActive;
  const canPan = editZoom > EDIT_ZOOM_MIN && !activeShapeTool && !smartTextIsActive;
...
function _bindCanvasPan(area) {
  ...
  area.addEventListener('mousedown', e => {
    if (e.button !== 0 || editZoom <= EDIT_ZOOM_MIN || activeShapeTool ||
        (typeof _smartTextActive !== 'undefined' && _smartTextActive)) return;
